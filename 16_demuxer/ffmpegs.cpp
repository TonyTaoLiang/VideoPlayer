...
static int decode(AVCodecContext *ctx,
    ...
    while (true) {
        ...
        qDebug() << "高度" << (ctx->height >> 1);

        qDebug() << "linesize" << frame->linesize[0] << frame->linesize[1] << frame->linesize[2];
        //320 *240 -》384 192  linesize[0]会变大。原因是源码static int update_frame_pool(AVCodecContext *avctx, AVFrame *frame)
